...
export default function Background({ density = 1, color = '11,61,46', boost = 1 }) {
  ...
  useEffect(() => {
    ...
    let W, H
    const C = color
    const B = boost            // global visibility multiplier
    const N = Math.floor(230 * density)
    ...
    const pts = Array.from({ length: N }, () => ({
      ...
      r: Math.random() * 2.3 + .6, op: Math.random() * .34 + .12,
    ...
    const draw = () => {
      ...
      ctx.strokeStyle = `rgba(${C},${.085 * B})`; ctx.lineWidth = 1
      ...
      for (let i = 0; i < 7; i++) {
        ...
        ctx.strokeStyle = `rgba(${C},${(.085 - i * .007) * B})`; ctx.lineWidth = 1; ctx.stroke()
      }
      ...
      for (const p of pts) {
        ...
        p.ph += p.ps; p.op = (.12 + Math.abs(Math.sin(p.ph)) * .3) * B
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        if (smx > 0) {
          ...
          if (d < 180) { ctx.beginPath(); ctx.moveTo(pi.x, pi.y); ctx.lineTo(smx, smy); ctx.strokeStyle = `rgba(${C},${(1 - d / 180) * .26 * B})`; ctx.lineWidth = .7; ctx.stroke() }
        }
        for (let j = i + 1; j < pts.length; j++) {
          const pj = pts[j], dx = pi.x - pj.x, dy = pi.y - pj.y, d = Math.sqrt(dx * dx + dy * dy)
          if (d < 120) { ctx.beginPath(); ctx.moveTo(pi.x, pi.y); ctx.lineTo(pj.x, pj.y); ctx.strokeStyle = `rgba(${C},${(1 - d / 120) * .17 * B})`; ctx.lineWidth = .55; ctx.stroke() }
        }
      }
  ...
  return (
    <>
      ...
      <div ref={glowRef} style={{ position: 'fixed', width: 500, height: 500, borderRadius: '50%', background: `radial-gradient(circle, rgba(${color},0.16) 0%, transparent 65%)`, transform: 'translate(-50%,-50%)', pointerEvents: 'none', zIndex: 0, mixBlendMode: 'multiply', transition: 'left 0.08s linear, top 0.08s linear', left: -999, top: -999 }} />
    </>
  )
}
